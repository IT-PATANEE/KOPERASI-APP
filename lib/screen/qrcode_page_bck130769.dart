  ...
  Future<void> fetchQrcodeData() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        _loans = (jsonResponse['data']['loans'] as List<dynamic>?) ??
            []; // เก็บข้อมูล accounts หากเป็น null ให้เป็นลิสต์ว่าง
        _arrahnu = (jsonResponse['data']['arrahnu'] as List<dynamic>?) ??
            []; // เก็บข้อมูล accounts หากเป็น null ให้เป็นลิสต์ว่าง
      });
    } else {
                    ...
                    return Container(
                      ...
                      child: ListTile(
                        ...
                        onTap: () {
                          setState(() {
                            _selectedAccount =
                                arrahnu; // กำหนดบัญชีที่เลือกให้กับ _selectedAccount
                            _toAccountNo = arrahnu[
                                'LCONT_ID']; // เก็บค่า ACCOUNT_NO ไว้ใน _selectedAccountNo
                          });
      ...
      child: Column(
        ...
        children: [
          ...
          if (otherMemberName.isNotEmpty) ...[
            ...
            if (toMem.length == 10) ...[
              const SizedBox(height: 24),
              _buildLoanSelectionSection(), // เรียกใช้ฟังก์ชันสลับหน้าตาปุ่ม / ช่องกรอกเงิน
            ],
          ],
        ],
      ),
                  ...
                  ? Column(
                      ...
                      children: [
                        ...
                        ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            final arrahnu = _arrahnu[index];
                            final isSelected = _selectedAccount == arrahnu;
                            ...
                            return AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(15), // ปรับเป็นโค้งมนมนิิมอลแบบ Type 3
                              ),
                              child: ListTile(
                                contentPadding: const EdgeInsets.symmetric(
                                ...
                                onTap: () {
                                  ...
                                  sendAccountDataToAPI(); // 🟢 Logic เดิมของ Type 4
                                },
                              ),
                            );
                          },
                        ...
                        const SizedBox(height: 24),
                        // 🟢 แสดงช่องกรอกจำนวนเงิน เฉพาะเมื่อเลือกบัญชีแล้ว
                        if (_selectedAccount != null) _buildAmountInputField(),
                      ],
                    )
                  : Center(
                      child: Padding(
  ...
  Widget _buildLoanSelectionSection() {
    ...
    return Column(
      ...
      children: [
        ...
        GestureDetector(
          onTap: () async {
            // 🟢 เด้งไปยังหน้า SelectToaccPage พร้อมส่ง List สัญญาทั้งหมดไปให้เลือก
            ...
            // 🟢 เมื่อเลือกสัญญาจากหน้าโน้นเสร็จ หน้าโน้นจะปิด (pop) และส่งข้อมูลกลับมาที่นี่
            if (result != null) {
              setState(() {
          ...
          child: isSelected
              ? _buildSelectedCardStyle() // ถ้าเลือกแล้วแสดงข้อมูลบัญชีตัวนั้น
              : _buildSelectorPlaceholder(), // ถ้ายังไม่เลือกแสดงปุ่มพร้อมไอคอน >
        ),
...
// ปุ่ม Placeholder ตอนยังไม่ได้เลือกสัญญา (มีไอคอน >)
  Widget _buildSelectorPlaceholder() {
    return Container(
...
// หน้าตาแบบกล่องการ์ด หลังจากที่ทำการเลือกสัญญาเสร็จสิ้นแล้ว
  Widget _buildSelectedCardStyle() {
    final String loanId =
        (_selectedAccount!['LCONT_ID'] ?? 'ไม่ระบุชื่อ').toString();
    final String regSalInt =
        (_selectedAccount!['LREG_SALINT'] ?? '0').toString();
    final String amountSal =
        (_selectedAccount!['LCONT_AMOUNT_SAL'] ?? '0').toString();
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(15),
        border: Border.all(
            color: Constants.greenColors, width: 2), // ขอบเน้นสีเขียว
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
